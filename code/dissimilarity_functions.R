  ...
  for (i in 1:ncol(pairs)){
    ...
    if (any(matr$Ordering=='ord')){
      ...
      sim.temp[m.o]<-abs(suppressWarnings(as.numeric(matr$Matrix[pairs[1,i],][m.o]))-
                           suppressWarnings(as.numeric(matr$Matrix[pairs[2,i],][m.o])))#/matr$MaxVals[m.o]
    }
    #correct values for unordered multistate characters
    if (any(matr$Ordering=='unord'&matr$MaxVals>1)){
      m.u<-which(matr$Ordering=='unord'&matr$MaxVals>1)
    ...
    wt.comp.char<-sum(na.omit(cbind(sim.temp,matr$Weights))[,2])
    HSJ[pairs[1,i],pairs[2,i]]<-HSJ[pairs[2,i],pairs[1,i]]<-sum(na.omit(sim.temp*matr$Weights))/wt.comp.char
  }
  diag(HSJ)<-0
  return(HSJ)
}
...
GED<-function(matr){
  ...
  for (i in 1:ncol(pairs)){
    ...
    #calculate total dissimilarity
    #wt.comp.char<-sum(na.omit(cbind(sim.temp,matr$Weights))[,2])
    GED[pairs[1,i],pairs[2,i]]<-GED[pairs[2,i],pairs[1,i]]<-sqrt(sum((sim.temp*matr$Weights)^2))
  }
  diag(GED)<-0
  return(GED)
}
...
gower<-function(matr){
  ...
  for (i in 1:ncol(pairs)){
    ...
    #calculate total dissimilarity
    wt.comp.char<-sum(na.omit(cbind(sim.temp,matr$Weights))[,2])
    gower[pairs[1,i],pairs[2,i]]<-gower[pairs[2,i],pairs[1,i]]<-sum(na.omit(sim.temp*matr$Weights))/wt.comp.char
  }
  diag(gower)<-0
  return(gower)
}